...
#Title:
#From Data to Diagnosis: Investigating the Factors Affecting Medical Insurance Costs
...
head(health_ins) # view dataset details
...
#summary statistics for the dataset, providing information like mean, median, quartiles, and counts for each variable.
summary(health_ins)

# dataframe df created by grouping the health_ins dataframe by the 'sex' column and calculating the count of each group.
df<- health_ins %>% group_by(sex) %>% summarise(count=n())
df

# categorical variable

##Analyzing distribution of insurance coverage among males and females in the dataset, 
#offering insights into the gender-based patterns of insurance enrollment.

#Below plot provides a clear visual comparison of the number of insured individuals in each gender category.
#The number of females and males are almost equal ,males being slightly higher than females in the dataset
...
#Histogram displaying the count of individuals with insurance coverage based on their age. The count of
#individuals with insurance in age group 20s have higher count as compared to others while count in 60s
#is the least amongst all the ages.
#This observation aligns with the idea that individuals in their 20s may be more inclined to secure
#insurance, possibly due to factors like increasing awareness of health and financial planning
...
##Analysis of proportion of male and female smokers in the 'health_ins' dataset.

#Barplot depicts the percentage of male and female smokers in the dataset. The analysis shows that
# propotion of smokers in male category are higher than the females 
...
#The below is the Boxplot distribution of insurance charges in the 'health_ins' dataset, 
#stratified by gender ('sex') and further differentiated by smoking status ('smoker')
#It allows for a quick comparison of central tendencies, variability, and the presence of outliers in  charges across these groups
# As observed from the boxplot the insurance charges are expectedly higher for smokers as compared to non smokers.
...
d<-ggplot(health_ins, aes(x=sex, y=charges,fill=smoker)) +
  geom_boxplot(outlier.color="red")   +
  theme_minimal()+
  scale_x_discrete(labels = c("Female", "Male")) +
  theme(legend.position="right")+
  ggtitle("Gender & Smoker-wise distribution of charges")+
  theme(plot.title = element_text(hjust=0.5))+
...
#The correlation coefficient between 'age' and 'bmi' is approximately 0.109 indicating a weak positive
# correlation between them.As age increases, BMI tends to slightly increase.

#The correlation coefficient between 'bmi' and 'charges' is approximately 0.198 indicating a weak 
# positive coorelation between them. As bmi increases, charges tends to slightly increase.

#The correlation coefficient between 'age' and 'charges' is approximately 0.299.This indicates  
#age and charges are moderately correlated and as age increases charges tend to increase.

e<-ggcorrplot(corr,lab=T)
ggplotly(e)

#one numerical variable distribution

##Analysis of  distribution of insurance charges in the dataset. 

# Histogram shows that the more people with 20000 insurance charges as compared to higher insurance charges
# The graph shows right-skewed distribution of insurance charges. This indicates that the proportion of individuals
# paying less charge is higher than individuals paying higher charges.This is intuitive also as people
#paying high insurance charges could be of factors like smoking,higher age group and geographical location.

options(scipen=999)

x<-ggplot(data=health_ins, aes(x=charges)) +
  geom_histogram(fill = "blue",bins=40, colour='white',aes(y=..density.. )) +
  labs(x ='Charges', y='', title = 'Insurance charges') +
...
# Distribution of charges across different regions, with a distinction between smokers and non-smokers. The median charges
#are highest in southeast for smokers followed by southwest. This shows that insurance charges in southern region is high
# as compared to northern regions. Interestingly, for non-smokers, the charges are in almost same range 
#across regions
...
# Using the charges attribute in this dataset the applicability of the central limit theorem can be shown. 
# There is  a right-skewed distribution of insurance charges in the dataset as displayed in the insurance charges histogram 
#Below are histograms showing the sample means of 1000 random samples of sample sizes 10, 30, 60, and 80 that
#follow a normal distribution. As sample size increases the distribution tends to be more normally distributed.
...
#Stratified sampling is when the larger group of data is broken into smaller stratas and then certain sizes are picked from each group. 

#We are analyzing the relationship between Body Mass Index (BMI), smoking status and insurance charges.
#The analysis is conducted on the entire population and using three different sampling methods as stated above.
...
# As seen in Scatterplot, the charges proportionately increase with increase in bmi for smokers while this is not the case with non smokers. In non smokers
...
#This contains information about the top two regions with the highest average charges.
#The dataset (health_ins) is grouped by the region and mean of the charges  is calculated for each region
...
cat("Top 2 Regions with Highest Average Charges:\n")
...
#Dataset is grouped by both children and sex variables.The mean of the age variable is calculated for each combination of children and sex.
...
#Conclusion:
#The data set explores the relationship between insurance charges and how they are affected by various attributes
# There are some observations in the dataset that might not be in sync with reality as the 
#author has taken into account specific attributes for analysis. There are social and environmental factors at play
# which might be the reason for data being skewed in some observations.Overall, the data reflects how
#personal and geographic attributes can affect health insurance charges.
